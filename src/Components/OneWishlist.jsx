...
const OneWishlist = ({ list }) => {
  ...
  const handleDelete = async (_id) => {
    try {
      const response = await fetch(
        `https://assignment-eleven-server-side-two.vercel.app/wishlist/${_id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div>
      <div className="md:grid grid-cols-5 text-black rounded-xl my-3 md:my-auto shadow-lg bg-pink-300 md:h-[210px]">
        <div className="col-span-2">
          <img
            className="w-full md:h-[210px] rounded-tr-xl rounded-tl-xl md:rounded-tl-xl md:rounded-bl-xl bg-gray-400"
            src={image}
            alt=""
          />
        </div>
        <div className="col-span-3 p-4  ">
          <div className="flex">
            <div className="flex-1">
              ...
              <p className="font-medium">Type: {coffee_type}</p>
              <div className="">
